...
class Predictor(nn.Module):
    ...
    def __init__(self, model, class_names, mean, std):
        ...
        self.transforms = nn.Sequential(
            T.Resize([256, ], antialias=False),  # We use single int value inside a list due to torchscript type restrictions
            T.CenterCrop(224),
...
def predictor_test(test_dataloader, model_reloaded):
    ...
    pred = []
    truth = []
    for x in tqdm(test_dataloader, total=len(test_dataloader.dataset), leave=True, ncols=80):
        softmax = model_reloaded(x[0])
